...
def show_info_window():
    ...
    info_text.insert(tk.END, "Important update from 18th January 2025. I realized that discord seems to have changed how the package folder works, I updated only the first main function of the program, because the other two are now easily replacable with discords search bar or /msg command. While I understand that this isn't a perfect solution especially if you cannot access your account. The function that isn't available on discord works now, so there's that. The previous version of the program still works fine if you're working with older packages, however new ones won't work, sorry for the inconvenience! Use version 2 for newer data, or the original version for older packages.\n\nIn order to use this program, you first need to request your data from Discord, then wait a few days until you get an email with your data. After you get the email, download the package.zip file and unzip it. Save the unzipped package folder location somewhere for ease of use.")
